...
func buildPrompt(profile profiles.Definition, prContext PullRequestContext) string {
	...
	b.WriteString("\n\n")
	b.WriteString("Return a structured review result with review_action, summary, general_comments, inline_findings, and warnings.\n")
	b.WriteString("Do not read files outside the current workspace. Do not inspect /nix/store or any other external directories.\n")
	b.WriteString("Do not use additional agents, subagents, or delegated reviews. Complete the review directly in the current workspace.\n")
	b.WriteString("If any tool call is denied or any permission request is rejected, continue the review using the repository content and context you already have instead of stopping.\n")
	b.WriteString("Even if some checks cannot be completed, still write the final structured JSON review result.\n")
